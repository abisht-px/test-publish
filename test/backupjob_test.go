...
func (s *PDSTestSuite) TestBackupJobDeletionFromTC_WithDissconnected_TC() {
	// Create a new deployment.
	deployment := api.ShortDeploymentSpec{
		DataServiceName: dataservices.Cassandra,
		ImageVersionTag: "4.1.2",
...
func (s *PDSTestSuite) TestBackupJobDeletionFromCP_WithDissconnected_TC() {
	...
	// dissconnect Target cluster
	deployment1, _ := s.targetCluster.GetDeployment(s.ctx, targetcluster.PDSChartNamespace, "pds-teleport")
	actualReplicas := deployment1.Spec.Replicas
	s.DisconnectTC(0, deployment1)
	s.controlPlane.DisconnectTestDeploymentTarget(s.ctx, s.T())

	// Delete backup, but it should not success
	s.controlPlane.MustDeleteBackupJobWithDisconnectTC(s.ctx, s.T(), backupJobId)
	backupJobTC := s.controlPlane.MustGetBackupJob(s.ctx, s.T(), backupJobId)
	s.Require().Equal(backupJobTC.GetId(), backupJobId)
	// connect TC
	deployment1, _ = s.targetCluster.GetDeployment(s.ctx, targetcluster.PDSChartNamespace, "pds-teleport")
	s.DisconnectTC(*actualReplicas, deployment1)
	s.controlPlane.MustWaitForDeploymentTarget(s.ctx, s.T(), s.config.pdsDeploymentTargetName)
...
func (s *PDSTestSuite) DisconnectTC(replicas int32, deployment *appsv1.Deployment) {
	deployment.Spec.Replicas = &replicas
	_, err := s.targetCluster.UpdateDeployment(s.ctx, targetcluster.PDSChartNamespace, deployment)
