...
func (s *PDSTestSuite) createBackupPolicy(name, schedule *string, retention *int32) (*pds.ModelsBackupPolicy, *http.Response, error) {
	...
	return s.controlPlane.PDS.BackupPoliciesApi.ApiTenantsIdBackupPoliciesPost(s.ctx, s.controlPlane.TestPDSTenantID).Body(requestBody).Execute()
}

func (s *PDSTestSuite) mustListBackupPolicy(backupPolicyID string) *pds.ModelsBackupPolicy {
	backupPolicy, resp, err := s.controlPlane.PDS.BackupPoliciesApi.ApiTenantsIdBackupPoliciesGet(s.ctx, s.controlPlane.TestPDSTenantID).Id2(backupPolicyID).Execute()
	api.RequireNoError(s.T(), resp, err)
...
func (s *PDSTestSuite) mustGetBackupPolicy(backupPolicyID string) *pds.ModelsBackupPolicy {
	backupPolicy, resp, err := s.controlPlane.PDS.BackupPoliciesApi.ApiBackupPoliciesIdGet(s.ctx, backupPolicyID).Execute()
	api.RequireNoError(s.T(), resp, err)
...
func (s *PDSTestSuite) updateBackupPolicy(backupPolicyID string, name, schedule *string, retention *int32) (*pds.ModelsBackupPolicy, *http.Response, error) {
	...
	return s.controlPlane.PDS.BackupPoliciesApi.ApiBackupPoliciesIdPut(s.ctx, backupPolicyID).Body(requestBody).Execute()
}
...
func (s *PDSTestSuite) deleteBackupPolicy(backupPolicyID string) (*http.Response, error) {
	return s.controlPlane.PDS.BackupPoliciesApi.ApiBackupPoliciesIdDelete(s.ctx, backupPolicyID).Execute()
}
