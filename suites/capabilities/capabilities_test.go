...
func (s *CapabilitiesTestSuite) TestCapabilities_DeleteDeploymentOperator() {
	...
	s.T().Cleanup(func() {
		...
		s.T().Run(fmt.Sprintf("Upgrade chart version %s", s.targetCluster.PDSChartConfig.Version), func(t *testing.T) {
	...
	s.Eventually(func() bool {
		...
		5*wait.ShortTimeout,
		3*wait.ShortRetryInterval,
		"deployment still exists, err: %v", err,
	)
	...
	s.Eventually(func() bool {
		...
		5*wait.ShortTimeout,
		3*wait.ShortRetryInterval,
		"operator capabilities still exists, err: %v", err,
	)
...
func (s *CapabilitiesTestSuite) TestCapabilities_UpdateDataServiceTLSFeatureFlag() {
	// Check that all operators' capabilities are exported to configmaps and reported to control plane.
	s.MustWaitForOperatorsToExportCapabilities()
	s.MustWaitForReportedCapabilities(expectedDefaultCapabilities)
...
func (s *CapabilitiesTestSuite) TestCapabilities_UninstallPDSChart() {
	// Check that all operators' capabilities are exported to configmaps and reported to control plane.
	s.MustWaitForOperatorsToExportCapabilities()
	s.MustWaitForReportedCapabilities(expectedDefaultCapabilities)
...
func (s *CapabilitiesTestSuite) MustWaitForOperatorsToExportCapabilities() {
	s.T().Helper()
...
func (s *CapabilitiesTestSuite) MustWaitForReportedCapabilities(expectedCapabilities pds.ModelsDeploymentTargetCapabilities) {
	s.T().Helper()
	s.Require().EventuallyWithT(func(t *assert.CollectT) {
		deploymentTarget := s.controlPlane.MustGetDeploymentTarget(s.ctx, s.T())
		reportedCapabilities := deploymentTarget.Capabilities
