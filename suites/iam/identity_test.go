...
func (s *IAMTestSuite) TestIdentityManagement_SanityCheck() {
	s.T().Skipf("need to fix this test")
	...
	authUserClient, err := api.NewPDSClient(s.ctx, s.ControlPlane.PDS.URL, authUserCredentials)
	require.NoError(s.T(), err, "initialize new PDS client")
	...
	serviceAccounts, response, err := authUserClient.ServiceAccountsApi.ApiTenantsIdServiceAccountsGet(
		context.Background(),
		s.ControlPlane.TestPDSTenantID,
	).Execute()
	...
	result, response, err := authUserClient.ServiceAccountsApi.ApiTenantsIdServiceAccountsGet(
		context.Background(),
		s.ControlPlane.TestPDSTenantID,
	).Execute()
	...
	clusters, response, err := authUserClient.DeploymentTargetsApi.ApiTenantsIdDeploymentTargetsGet(
		context.Background(),
		s.ControlPlane.TestPDSTenantID,
	).Execute()
	api.RequireNoErrorWithStatus(s.T(), response, err, http.StatusOK)
	...
	// List the deployment targets.
	targets, response, err := authUserClient.DeploymentTargetsApi.ApiTenantsIdDeploymentTargetsGet(
		context.Background(),
		s.ControlPlane.TestPDSTenantID,
	).Execute()
	api.RequireErrorWithStatus(s.T(), response, err, http.StatusForbidden)
